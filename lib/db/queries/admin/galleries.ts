...
export const getGalleries = async (page: number, published: string): Promise<{ galleries: Gallery[]; perPage: number; galleryCount: number }> => {
...
export const getGalleryStatsCount = async () => {
  try {
...
export const getGalleryImageByUrl = async (url: string) => {
  try {
