...
const MaxPurchaseField = () => {
  ...
  return (
    <FormFieldWrapper>
      <FieldLabel htmlFor="maxPurchase">Maximum Purchase Quantity</FieldLabel>

      <div className="w-full flex-col space-y-2">
        <FieldInput disabled={isSubmitting} name="maxPurchase" register={register} type="number" id="maxPurchase" required={true} />
        <span className="text-[12px] text-muted-foreground">* Max number {maxStockAllowed} unit or lower allowed</span>{" "}
        {errors?.maxPurchase && <FieldError error={errors?.maxPurchase?.message as string} />}
      </div>
    </FormFieldWrapper>
  );
};

export default MaxPurchaseField;
